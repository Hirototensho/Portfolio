...
# knitr::purl("stats-note2.qmd", documentation = 0) # Convert R Markdown to R script
# reprex::reprex_clean()
# reprex::reprex(advertise = FALSE)
# reprex::reprex(advertise = FALSE, wd = getwd())

# options(reprex.highlight.hl_style  = "edit-xcode", reprex.highlight.font_size = 9)
# reprex::reprex_clean()
# reprex::reprex(advertise = FALSE, venue = "rtf")
```
...
具体的には、線形モデルの説明変数 $\boldsymbol{X}_i$ には教育年数と勤続年数を使い、プロビットモデルの説明変数 $\boldsymbol{Z}_i$ には教育年数と勤続年数に加えて子どもの人数を使う、といった配慮が必要です。
...
ここでは @exm-Heckit-example3 で推定したタイプIIトービットモデルの限界効果について、ブートストラップ法を使った標準誤差の推定と検定を試みたいと思います。まず、 @exm-Heckit-example1 で実装したモデル推定し、@exm-Heckit-example2 で実装した `heckitmfx()` 関数で限界効果を推定するまでの操作を、ひとまとめにしたラッパー関数を定義します。
...
```{r}
#| echo: false
# heckitmfx_with_boot |> write_csv(here::here("output", "heckitmfx_with_boot.csv"))
# heckitmfx_with_boot <- read_csv(here::here("output", "heckitmfx_with_boot.csv"), show_col_types = FALSE)
